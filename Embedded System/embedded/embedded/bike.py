...
class Bike:
    ...
    @property
    def public_key(self):
        return self.signing_key.get_verifying_key()


    def sign(self, data):
        return self.signing_key.sign(data) #Removed dedicated return type, check encoding type


    async def lock(self, request_id):
        self.lockedLongitude = self.longitude
        self.lockedLattitude = self.lattitude
        self.locked = True
    ...
    def update_coordinates(self, longitude, lattitude):
        self.totalDistance =+ self.haversine(self.lattitude,self.longitude,lattitude,longitude) #Updates total distance gone
